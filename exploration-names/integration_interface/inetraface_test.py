...
st.sidebar.divider()
use_biomart = st.sidebar.checkbox("Add BioMart annotations", value=False)
...
@st.cache_resource
def cached_load_dataset_from_path(path: str, use_biomart=False, organism="hsapiens"):
    adata = sc.read_h5ad(path)

    #ajout de base_name et has_versions
    adata = gene_versions(adata)

    note = None

    #ajout des colonnes biomart si demandé
    if use_biomart:
        if "gene_ids" not in adata.var.columns:
            note = "BioMart non appliqué : colonne 'gene_ids' absente."
        else:
            try:
                adata = add_biomart_names(adata, organism=organism)
                note = "BioMart applied successfully."
            except Exception as e:
                note = f"BioMart non appliqué : {e}"
...
@st.cache_resource
def cached_load_dataset_from_bytes(file_bytes: bytes, use_biomart=False, organism="hsapiens"):
    import tempfile
...
if load_clicked:
    if not data_path:
        ...
        try:
            ds, rep = cached_load_dataset_from_path(data_path, use_biomart=use_biomart)
...
def gene_exists(adata, gene: str) -> bool:
    ...
    return result["status"] in ["exact", "multiple"]
...
with tab1:
    ...
    ####Changement### Pour choix multiple
    if "single_gene_result" not in st.session_state:
        st.session_state.single_gene_result = None

    if "single_gene_query" not in st.session_state:
        st.session_state.single_gene_query = None
    ...
    # Si l'utilisateur tape un gène mais n'a pas encore lancé la recherche
    if gene and adata is not None and not run_single:
        preview_result = resolve_gene_for_streamlit(adata, gene)

        if preview_result["status"] == "suggestions" and preview_result["suggestions"]:
            st.caption("Suggestions: " + ", ".join(preview_result["suggestions"]))

        elif preview_result["status"] == "multiple" and preview_result["hits"] is not None:
            options = hits_to_options(preview_result["hits"])
            if options:
                st.caption("Possible matches: " + ", ".join([x[0] for x in options[:5]]))

    # Quand on clique Run : on stocke juste le résultat
    if run_single:
        if adata is None:
            ...
            if "single_gene_select" in st.session_state:
                del st.session_state["single_gene_select"]
            if "single_gene_suggestion_select" in st.session_state:
                del st.session_state["single_gene_suggestion_select"]

            st.session_state.single_gene_result = resolve_gene_for_streamlit(adata, gene)
            st.session_state.single_gene_query = gene

    # Affichage du résultat stocké
    result = st.session_state.single_gene_result
    ...
    if result is not None:
        ...
        if result["status"] == "none":
            ...
            st.warning("No exact match found. Please choose the intended gene.")
            ...
            if options:
                ...
                selected_label = st.selectbox(
                    "Suggested genes",
                    labels,
                    key="single_gene_suggestion_select"
                )
                selected_gene = label_to_var[selected_label]

                v = get_gene_vector(adata, selected_gene)
                ...
                if show_hist:
                    with col1:
                        fig = plot_hist(v_plot, f"{selected_gene} — Histogram ({scale_mode})")
                        st.pyplot(fig, clear_figure=True)

                if show_violin:
                    with col1:
                        fig = plot_violin(v_plot, f"{selected_gene} — Violin ({scale_mode})")
                        st.pyplot(fig, clear_figure=True)

                if show_umap_opt:
                    with col2:
                        if "X_umap" not in adata.obsm.keys():
                            st.warning("UMAP not found in this dataset (obsm['X_umap'] missing).")
                        else:
                            fig = plot_umap(adata, v_plot, f"{selected_gene} — UMAP colored by expression")
                            st.pyplot(fig, clear_figure=True)
            ...
            st.warning("Multiple matches found. Please choose one gene/version.")

            matched_text = None
            if result.get("match_type") == "partial":
                 matched_text = st.session_state.single_gene_query
        ...
        elif result["status"] in ["exact", "partial"]:
